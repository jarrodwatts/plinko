...
contract PlinkoGameTest is Test {
    ...
    function testPlayRoundInvalidNonce() public {
        ...
        bytes memory signature = _createSignature(player1, betAmount, randomSeed, multiplier, wrongNonce);
        
        vm.prank(player1);
        vm.expectRevert(PlinkoGame.InvalidNonce.selector);
        plinkoGame.playRound{value: betAmount}(randomSeed, multiplier, wrongNonce, signature);
    }
    
    function testPlayRoundInvalidMultiplier() public {
        uint256 betAmount = 0.1 ether;
        uint256 randomSeed = 12345;
        uint256 invalidMultiplier = 999; // Not in MULTIPLIERS array
        uint256 nonce = 0;
        
        bytes memory signature = _createSignature(player1, betAmount, randomSeed, invalidMultiplier, nonce);
        
        vm.prank(player1);
        vm.expectRevert(PlinkoGame.InvalidMultiplier.selector);
        plinkoGame.playRound{value: betAmount}(randomSeed, invalidMultiplier, nonce, signature);
    }
    ...
    function testPlayRoundInvalidSignature() public {
        ...
        bytes memory wrongSignature = _createSignatureWithKey(wrongPrivateKey, player1, betAmount, randomSeed, multiplier, nonce);
    ...
    function testFuzzPlayRound(uint256 betAmount, uint256 randomSeed, uint8 multiplierIndex) public {
        ...
        uint256 multiplier = plinkoGame.MULTIPLIERS(multiplierIndex);
        uint256 nonce = 0;
    ...
    function _createSignature(
        ...
        return _createSignatureWithKey(serverPrivateKey, player, betAmount, randomSeed, multiplier, nonce);
    }
    ...
    function _createSignatureWithKey(
        ...
        bytes32 messageHash = keccak256(abi.encodePacked(player, betAmount, randomSeed, multiplier, nonce));
